...
use App\Models\{Venue, VenueSloting, VenueAddress, Vistors, Country,  Timezone, Reason, JobStatus};
...
class VisitorBookingController extends Controller
{
    ...
    public function WaitingPage(Request $request)
    {
        ...
        if ($tokenStatus['status']) {
            ...
            $userInputs['inputs']['slotId'] = $slotId;
            $userInputs['inputs']['tokenId'] = $tokenId;
            $userInputs['inputs'] = $request->all();
        } else {
            return response()->json([
                'errors' =>  $tokenStatus
            ], 422);
    ...
    {
        return view('frontend.multistep.inc.waiting');
    }
